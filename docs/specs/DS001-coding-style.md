...
The repository may expose alternate visual modes through lightweight URL parameters and client-side persistence when those modes do not alter the information architecture or page content. The current presentation workflow keeps the shared theme picker coordinated through shared bootstrap logic and shared CSS rather than page-specific forks. The `inspiration` presentation is the default site theme, while the older `default` presentation and the `fusion` review presentation remain accessible through the Dev Preview control and direct `?theme=` links. Alternate modes must avoid framework dependencies.
