...
The site must present a restrained institutional aesthetic rooted in Swiss, biomedical, and technical design cues. The default LydiaRX presentation is the `inspiration` theme; it may use tightly controlled genomic motion, but it must still feel serious, research-led, and subordinate to the content rather than playful or startup-marketing oriented. Alternate review themes may introduce controlled motion only when it remains subordinate to the content and does not reduce legibility.

The design system must derive its primary palette from the LydiaRX logo and the live LydiaRX brand site. The canonical anchors are the LydiaRX blue-green title color, restrained green support accents, pale blue-white surfaces defined as reusable CSS variables in `docs/styles.css`, and a darker near-black reserved for body copy and dense reading contexts. Large display titles, section headings, card titles, primary navigation labels, and the header slogan should use the LydiaRX green title color rather than neutral black or mismatched blue so the visual hierarchy stays recognisably tied to the logo across the public site. Green primary buttons must use white text, and outlined or secondary buttons must keep enough contrast in default and hover states. Public pages should use bright, luminous surfaces and subtle brand tinting rather than heavy dark backgrounds, and those colors must remain visibly repeated across navigation, buttons, dropdown menus, cards, highlights, diagrams, consent surfaces, and major title treatments.
...
The `inspiration` visual system is the authoritative default presentation. The site may also expose explicitly selectable review or fallback themes when those variants preserve the same content structure and page hierarchy. The current selectable themes are `default`, `fusion`, and `inspiration`, and the shared corner review widget should be visible by default unless a stored override explicitly disables it. Alternate themes must remain visibly coherent across public pages, technical pages, navigation, consent UI, dialogs, and diagrams rather than only recoloring isolated components. The `inspiration` theme should use one shared low-contrast animated helix-style point-field system on its selected white tower floors, with randomised seeds per page, floor, and refresh so the layouts regenerate uniquely on each run. The hero floor should concentrate larger and more legible double-helix structures on the right side, using visible base-pair rungs, sequenced pulse highlights, strand particles, and subtle pointer-responsive glow or deflection. The left and reading zones should retain only a heavily veiled continuation of the field so the animation feels continuous without competing with text. Secondary white floors may keep smaller genomic clusters and ambient particles. The motion layer must always remain behind fully opaque content, above a white base surface, and below a left-to-right white reading mask that keeps the text edge calm.
